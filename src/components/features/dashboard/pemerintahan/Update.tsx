...
interface UpdateVillageGovernmentProps {
  state: {
    status: string;
    errors?: {
      title?: string[];
  ...
  isPending: boolean;
  onCancel?: () => void;
  updateForm: TablesUpdate<"village_government">;
  setUpdateForm: React.Dispatch<
    React.SetStateAction<TablesUpdate<"village_government">>
  >;
}

export default function UpdateVillageGovernment({
  state,
  formAction,
  isPending,
  onCancel,
    ...
    if (files && files?.length > 0) {
      ...
      try {
        const { path, token } = await createImageSignedUrl(
          Bucket.VillageGovernment
        );
        const imageUrl = await uploadFileToSignedUrl({
          bucket: Bucket.VillageGovernment,
          file,
          path,
          token,
        });
    ...
    <form action={formAction} className="grid gap-4 mt-4">
      ...
      <div className="grid gap-2">
        ...
        <Input
          ...
          placeholder="Masukkan judul struktur pemerintahan desa"
        />
        {state.errors?.title && (
      ...
      <div className="grid gap-2">
        ...
        <Textarea
          ...
          placeholder="Masukkan deskripsi struktur pemerintahan desa"
          rows={3}
        />

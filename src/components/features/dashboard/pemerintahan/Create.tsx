...
interface CreateVillageGovernmentProps {
  state: {
    status: string;
    errors?: {
      title?: string[];
...
export default function CreateVillageGovernment({
  state,
  formAction,
  isPending,
  onCancel,
}: CreateVillageGovernmentProps) {
  ...
  const handleChangeImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files?.length > 0) {
      ...
      try {
        ...
        setImgUrl(imageUrl);
        toast.success("Gambar struktur pemerintahan desa berhasil diupload!");
      } finally {
